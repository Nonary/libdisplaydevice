...
namespace display_device {
  namespace {
     ...
     */
    bool doSetModes(WinApiLayerInterface &w_api, const DeviceDisplayModeMap &modes, const Strategy strategy, bool persist = true) {
  ...
  bool WinDisplayDevice::setDisplayModes(const DeviceDisplayModeMap &modes) {
    ...
    if (all_device_ids.empty()) {
      // This can occur if active path enumeration is temporarily inconsistent; log as warning.
    ...
    if (!doSetModes(*m_w_api, modes, Strategy::Relaxed, true)) {
      // Error already logged
      return false;
    }
    ...
    if (!current_modes.empty()) {
      ...
      if (doSetModes(*m_w_api, modes, Strategy::Strict, true)) {
  ...
  bool WinDisplayDevice::setDisplayModesTemporary(const DeviceDisplayModeMap &modes) {
    ...
    // Apply relaxed strategy first without saving to database
    if (doSetModes(*m_w_api, modes, Strategy::Relaxed, false)) {
      return true;
    }

    // If relaxed application failed, attempt strict application (still temporary)
    return doSetModes(*m_w_api, modes, Strategy::Strict, false);
  }
  ...
  bool WinDisplayDevice::setDisplayModesWithFallback(const DeviceDisplayModeMap &modes) {
    ...
    // Best-effort: allow OS to choose closest supported mode; persist changes
    if (doSetModes(*m_w_api, modes, Strategy::Relaxed, true)) {
      return true;
    }

    // If OS couldn't adjust suitably, try strict application and persist
    return doSetModes(*m_w_api, modes, Strategy::Strict, true);
  }
}  // namespace display_device
